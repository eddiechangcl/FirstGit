601,100
602,"}Drill_drill to security cube entry"
562,"VIEW"
586,"}CubeSecurity"
585,"}CubeSecurity"
564,
565,"e2ckWaJ:J8fkq[d_LU?VK]F3v6P`VQLxYQ@3_<c3m5dmtXnYM@FE83XP>Ab\Y4ywA227ocWz5^HmFmr1oi@;iK1ZfeZb;FFHb8<P4aFaRa0B>P3F;R8_C;jq;w`I[_I[k>EL[Rhf=HLkQGu2FCsn3pz@1IjU:A_fFm9MM?ofInfaKFX]A5GIoY@LZ7FpnzJ6M1bVUH4K"
559,1
928,1
593,
594,
...
######################
### Purpose:
### Drill through to TM1 security cube from security audit cube
...
cDimSrc = '}APQ TM1 Object Type';
cCubTgt = AttrS( cDimSrc, pType, 'SysSecObjNm' );
cDimTgt = AttrS( cDimSrc, pType, 'SysObjNm' );

cViewTgt = 'zDrill to Security Cube';
cSubTgt = cViewTgt;
...
# Set title dimension as the control list dimension
ViewTitleDimensionSet( cCubTgt, cViewTgt, cDimTgt );
# Set element as pObject
If( SubsetExists( cDimTgt, cSubTgt ) = 0 );
  SubsetCreate( cDimTgt, cSubTgt );
Else;
  SubsetDeleteAllElements( cDimTgt, cSubTgt );
EndIf;
SubsetElementInsert( cDimTgt, cSubTgt, pObject, 1 );
ViewSubsetAssign( cCubTgt, cViewTgt, cDimTgt, cSubTgt );

# Set row dimension as the groups list dimension
ViewRowDimensionSet( cCubTgt, cViewTgt, '}Groups', 1 );
If( SubsetExists( '}Groups', cSubTgt ) = 0 );
  SubsetCreate( '}Groups', cSubTgt );
EndIf;
SubsetIsAllSet( '}Groups', cSubTgt, 1 );
If( DimIx( '}ElementAttributes_}Groups', '}TM1_DefaultDisplayValue' ) > 0 );
  SubsetAliasSet(  '}Groups', cSubTgt, '}TM1_DefaultDisplayValue' );
EndIf;
ViewSubsetAssign( cCubTgt, cViewTgt, '}Groups', cSubTgt );
...
IF( 1 = 0 );
#****Begin: Generated Statements***
RETURNVIEWHANDLE('}CubeSecurity','zDrill to Security Cube');
#****End: Generated Statements****
