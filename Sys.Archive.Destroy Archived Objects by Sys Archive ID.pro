601,100
602,"Sys.Archive.Destroy Archived Objects by Sys Archive ID"
562,"NULL"
586,
585,
564,
565,"xk6Y0^FfQPm3NFw@B_ZW^6HMzKP`k<Mpc?]c9wTPKrgzI""?]tz<P2yD>ukER;qBz1BxNdYpwnncYk?FkHZRnxEwXo<pcqKJCukSeUUzVPw;DCyR2epM:TtY?nY@MqDUf14EG]D@A:0TMbrIXJpp-?ATsGzTdWKYo^G9BclrVc<zt`EVs6@SCYn6Gb?_40A?U3@u4Z_gxk6Y0^FfQPm3NFw@B_ZW^6HMzKP`k<Mpe?]c9wTPKrgzIb7]tz<P2yD>uK=U;qRx1Bx~fYpwnnC_koEkHZRnxEwXo<`jqKJCukSeXUzVPw;DCyBodpM:TtY?nY@MAC\f1dAwUD@A:0TMcrIHJpp=5ATsGzTdWKYo^GIOclrQc<zt`HVs6@SCY~6Gb?_4016OW3@u4Z_gxk6Y0^FfQPm3NFw@B_ZW^6HMoKP`k<M@j?]c9wTPKrgzIR8]tz<P2yD>uKaY;qb{1Bx>hYpwnnSTk_JkHZRnxEwXo<p`qKJCukSEZUzVPw;DCyBjipM:TtY?nY@MARSf1tNG[D@A:0TmmrIHApp=7ATsGzTdWKYo^GyBclrRc<zt0BVs6@SCYn3Gb?_40a:O_3@u4Z_g"
559,1
928,0
593,
594,
...
cTempSubset= cProcessName | '.' | cTimeStamp | '.' | sRandomInt ;

cCube = 'Sys Archive Profile' ;
cMDeletedBy = 'Sys Archive ID Deleted By' ;
cMDeletedTime = 'Sys Archive ID Deleted Time' ;

cDeleteBy = pRunBy;
IF( DimIx( '}Clients', cDeleteBy ) = 0 );
   cDeleteBy = TM1User();
Endif;
IF( DimIx( '}Clients', cDeleteBy ) > 0 );
  cDeleteBy = IF( AttrS( '}Clients', cDeleteBy, '}TM1_DefaultDisplayValue' ) @= '', cDeleteBy, AttrS( '}Clients', cDeleteBy, '}TM1_DefaultDisplayValue' ) );
EndIF;

cDeleteTime = TimSt(Now(), '\Y-\m-\d \h:\i:\s' );
...
#######################################################################
#Destroy Archvied Cubes

sTempDim = cDimCubes ;
sTempMDX = '{TM1FILTERBYPATTERN( {TM1SUBSETALL( [' | sTempDim | '] )}, "zArchived_*' | cElemArchiveID | '")}' ;

If (SubsetExists( sTempDim, cTempSubset ) <> 0);
     SubsetDestroy( sTempDim, cTempSubset );
Endif;
SubsetCreatebyMDX( cTempSubset, sTempMDX, sTempDim );
nElements = SubsetGetSize( sTempDim, cTempSubset );
sLastElement = SubsetGetElementName(sTempDim, cTempSubset, nElements);
SubsetElementDelete(sTempDim, cTempSubset, nElements);
SubsetElementInsert(sTempDim, cTempSubset, sLastElement, nElements);
nElements = SubsetGetSize( sTempDim, cTempSubset );

nIndex = nElements;
WHILE( nIndex >= 1 );

    sTempElemName = SubsetGetElementName( sTempDim, cTempSubset, nIndex );
    IF( CubeExists( sTempElemName ) = 1 );
        CubeDestroy( sTempElemName );
    ENDIF;
...
#######################################################################
#Destroy Archived Dimensions

sTempDim = cDimDimensions ;
sTempMDX = '{TM1FILTERBYPATTERN( {TM1SUBSETALL( [' | sTempDim | '] )}, "zArchived_*' | cElemArchiveID | '")}' ;
...
WHILE( nIndex >= 1 );
    ...
    sTempElemName = SubsetGetElementName( sTempDim, cTempSubset, nIndex );
    IF( DimensionExists( sTempElemName ) = 1 );
        DimensionDestroy( sTempElemName );
    ENDIF;
...
#Sys Archive ID Information
sM_Name01=cMDeletedBy; sM_Value01=cDeleteBy;
sM_Name02=cMDeletedTime; sM_Value02=cDeleteTime;

nnIndex = 1;
WHILE( nnIndex <= 2 );
     IF( CellIsUpdateable( cCube, cElemArchiveID, cCube, Expand( '%sM_Name'| NumberToStringEx( nnIndex, '00', '', '' ) | '%' ) ) = 1 );
          CellPutS( Expand( '%sM_Value' | NumberToStringEx( nnIndex, '00', '', '' ) | '%' ),  cCube, cElemArchiveID, cCube, Expand( '%sM_Name'| NumberToStringEx( nnIndex, '00', '', '' ) | '%' ) );
     ENDIF;
     nnIndex = nnIndex + 1;
END;
