601,100
602,"Sys.Archive.Export Cube Rules by Sys Archive ID"
562,"CHARACTERDELIMITED"
586,"D:\DBs\HOI\Data\Sys Menu.RUX"
585,"D:\DBs\HOI\Data\Sys Menu.RUX"
564,
565,"fmbKF[zh4V?50B][cdEL>Zsd[r1[AP1v3l]onjv`uNL:\'OJw\3p\94bLdEr4OHZxPU@dtUGT>hYT:VnCn0QzPaL\^sCpA>7dsKnUJWBePq_`]_2E1r:zHKZ\FviuD5FY6uD=EQvZO\KRWvV:b['O@yPGUUcee`3Wq6rAUsf?Joiceenbhw=TafZA0B`X@Ove@vDZ<CkfmbKF[zh4V?50B][cdEL>Zsd[r1[AP1v5l]onjv`uNL:\gGJw\3p\94bLD=u4OXXxPUpftUGT>H_TjUnCn0QzPaL\^cJpA>7dsKnXJWBePq_`]OoD1r:zHKZ\FviEC<FYfqt5EQvZO\KSWvF:b[7E@yPGUUcee`3WqFAUsa?Joichenbhw=TqfZA0B`X0FFg@vDZ<CkfmbKF[oh4V?50B][cdEL>Zsd[r1[AP1V3l]onjv`uNL:\WHJw\3p\94bLDqq4Oh[xPU0htUGT>XTTJWnCn0QzPaL\^s@pA>7dsKnRJWBePq_`]OjI1r:zHKZ\FvieM<FYVrD;EQvZO\;ZWvV5b[WG@yPGUUcee`3WqvrAU3i?Joi3benbhw=TacZA0B`X`JFo@vDZ<Ck"
559,1
928,0
593,
594,
...
cElemArchiveCube = pArchiveCube ;

#Export File 
cExportFolder = CellGetS( cCubeConfig, 'Export Folder', 'Text' ) ;
IF (SUBST(cExportFolder, Long(cExportFolder), 1) @= '\'); cExportFolder = SubSt(cExportFolder, 1, Long(cExportFolder) - 1); ENDIF;
cExportFolder = cExportFolder | '\Archive Rule Files\' ;
cExportFilePath = cExportFolder | 'zArchived_' | pArchiveCube | '_' | pArchiveID | '.txt';
...
cSourceFilePath = cDataFolder | '\' | pArchiveCube | '.rux';

#######################################################################
#Validations

#Validate Cube
If ( Trim(cElemArchiveCube) @= '' % CubeExists( cElemArchiveCube ) = 0 );
    sError = 'Invalid Cube :' | cElemArchiveCube ;
...
IF( FileExists( cSourceFilePath  ) = 0 );
    sError = 'No Rule File found for cube:' | cElemArchiveCube ;
    AsciiOutput(sDebugFile, sError);
    If (sErr @= '');
        sErr = sErr | Char(10) | Char(13);
    Endif;
...
sArchiveRuleFlag = CellGetS( 'Sys Archive Profile', cElemArchiveID, cElemArchiveCube, 'Archive Rules Flag' );

IF( sArchiveRuleFlag @= '' );
    sError = 'Not set to export rules for cube:' | cElemArchiveCube ;
    AsciiOutput(sDebugFile, sError);
    If (sErr @= '');
        sErr = sErr | Char(10) | Char(13);
    Endif;
...
ASCIIOUTPUT( cExportFilePath, V1 );
