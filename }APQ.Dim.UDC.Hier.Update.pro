601,100
602,"}APQ.Dim.UDC.Hier.Update"
562,"VIEW"
586,"}APQ Dimension Static Subsets And UDC"
585,"}APQ Dimension Static Subsets And UDC"
564,
565,"rRJ[rSrQlC5=`cY44Gadc6hsTDm^k9yw_?Ybs^zV6ZlH_]W^hmn<^@1b]vgeoNsm8]neg;s9aSOdA3xWc\Fnq`NB5aJ<AB3>V6Im`zdj3Xhu`flE\vkc>@UhOjfLRYHO6>FuLEoLNTotX:ItoqSnJ?xr@3DNcR?GLbJ>TJP<DDQ^=LNYx6@B^7m>9m\Ur7[Rtmjpm\j5"
559,1
928,0
593,
594,
...
######################
### Purpose:
### Update hierarchical Parent/Child user defined consolidations
...
sPar    = CellGetS( cCubSrc, vDim, cType, vSubIdx, vItemIdx, 'NodeName' );
IF( sPar @= '' );
  ItemSkip;
EndIF;
...
IF( Scan( ':', sHier ) > 0 );
  vDimHier = sHier;
  # due to a bug in PA if a leaf element and alias used will only be recognized in same named hierarchy & Leaves hierarchy. Need to check for principal name
  IF( DimIx( vDim | ':Leaves', vEle ) > 0 );
    vEle = DimNm( vDim | ':Leaves', DimIx( vDim | ':Leaves', vEle ) );
  EndIF;
Else;
  vDimHier = vDim;
EndIF;

# Unwind UDC (only executed for 1st item of defined rollup)
# to be safe can't call Bedrock.Dim.Hierarchy.Unwind.Consolidation since users could put high level standard consols as part of UDC. 
# Just delete all children of the UDC placeholder node. Should more complete unwind be needed this will need to be handled elsewhere in custom process
...
### Add elements to hier UDC
IF( DimIx( vDimHier, vEle ) > 0 );
  vEle = DimensionElementPrincipalName( vDimHier, vEle );
Else;
  DimensionElementInsertDirect( vDimHier, '', vEle, 'N' );
EndIF;
IF( DimIx( vDimHier, sPar ) > 0 );
  sPar = DimensionElementPrincipalName( vDimHier, sPar );
Else;
  DimensionElementInsertDirect( vDimHier, '', sPar, 'C' );
EndIF;

# Check for circular reference
If( sPar @= vEle % ElIsAnc( vDimHier, vEle, sPar ) = 1 );
    LogOutput( 'WARN', Expand('%sThisProcName% circular dimension reference detected at %vSubIdx%:%vItemIdx%,%vDimHier%,%sPar%,%vEle%') );
    ItemSkip;
Else;
    nWht = CellGetN( cCubSrc, vDim, cType, vSubIdx, vItemIdx, 'ElementWht' );
    DimensionElementComponentAddDirect( vDimHier, sPar, vEle, nWht );
EndIf;
CellPutN( Now(), cCubTgt, vDim, cType, vSubIdx, '0001', 'LastConstructN' );

# UDC root node name substitution
...
# check if vEle = UDC top node OR if sPar = UDC top node (if yes add to UDC root node)
IF( vEle @= sUDC % sPar @= sUDC );
    IF( DimIx( vDimHier, sUDCNode ) = 0 );
        DimensionElementInsertDirect( vDimHier, '', sUDCNode, 'C' );
    EndIF;
    DimensionElementComponentAddDirect( vDimHier, sUDCNode, sUDC, 1 );
    # only if the herarchy root node exists then add UDC root note to the hierarchy root (with 0 weight since only for org purposes)
