601,100
602,"Sys.Workflow.Cooling Period.Update Status"
562,"NULL"
586,
585,
564,
565,"qT9fpZX:Y3WSd>0g4aK<kqmkrdfP?>QEIF2N;0a;s6mEHdO:MfQqyz7jNOqUmVUIaZj0gn=l5[=Xp@?T6XS]XAs_0EfMP2rqZsxK4F=p[Bbr`5L[@E3zqvPxp?pJkJr;GqGjyObiR6tDwMW;\P<cy:RmV4U[n:aqZ=Ye96lR9`Syz]qPuFPc<5[0EO_tvAEl;sySgoIb"
559,1
928,0
593,
594,
...
pYear,"Year:"
pWorkflow,"Workflow:"
pEnableCooling,"Enable Cooling: (Optional, blank as DISABLE, ✓ as ENABLE)"
577,0
578,0
579,0
580,0
...
cDimMeasure = 'M Sys Workflow Cooling Period';

cScenario = pScenario;
cYear = pYear;
cWorkflow = pWorkflow;
cEnableCoolingUpdate = pEnableCooling;

cLineItemInput = 'INPUT LINE';
cEnableCooling = 'ENABLE COOLING PERIOD';

cCube = 'Sys Workflow Cooling Period';
...
CellPutS(cEnableCoolingUpdate, cCube, cScenario, cYear, cWorkflow, cLineItemInput, 'ENABLE COOLING PERIOD');
If (cEnableCoolingUpdate @= '✓');
    CellPutS(sNow, cCube, cScenario, cYear,  cWorkflow, cLineItemInput, 'LAST ENABLE COOLING UPDATED');
    CellPutS(sClient, cCube, cScenario, cYear, cWorkflow, cLineItemInput, 'LAST ENABLE COOLING UPDATED BY');
Else;
    CellPutS(sNow, cCube, cScenario, cYear, cWorkflow, cLineItemInput, 'LAST DISABLE COOLING UPDATED');
    CellPutS(sClient, cCube, cScenario, cYear, cWorkflow, cLineItemInput, 'LAST DISABLE COOLING UPDATED BY');
Endif;

##############################################################################
#
# UPDATE EACH LINE FOR RECORD KEEPING
#
##############################################################################
...
While (nLineItem <= nLineItemMax);
     ...
     sLineEnableCooling = CellGetS(cCube, cScenario, cYear, cWorkflow, sLineItem, 'LAST ENABLE COOLING UPDATED');
     sLineDisableCooling = CellGetS(cCube, cScenario, cYear, cWorkflow, sLineItem, 'LAST DISABLE COOLING UPDATED');
     If (sLineEnableCooling @= '' & sLineDisableCooling @= '');
         nCountMeasure = 1;
         While (nCountMeasure <= nMeasures);
              sMeasure = DimNM(cDimMeasure, nCountMeasure);
              sValue = CellGetS(cCube, cScenario, cYear, cWorkflow, cLineItemInput, sMeasure);
              CellPutS(sValue, cCube, cScenario, cYear, cWorkflow, sLineItem, sMeasure);
...
######################
### Clear Current Workflow Disable Cooling Config
sProcess = 'Cub.WF_Workflow.Update Disable Cooling';
sParam1Name = 'pDoProcessLogging';
sParam1Value = pDoProcessLogging;
sParam2Name = 'pScenario';
sParam2Value = cScenario;
sParam3Name = 'pYear';
sParam3Value = cYear;
sParam4Name = 'pWorkflow';
sParam4Value = cWorkflow;
sParam5Name = 'pNode';
sParam5Value = 'All Cost Centers';
sParam6Name = 'pDisableCooling';
sParam6Value = '';
