601,100
602,"Sys.Localization.CaptionAsAlias"
562,"NULL"
586,
585,
564,
565,"rbSFvUQS3ng9D]8IPea[bKHm3Q`XNubRu4>gPjUg:WB[QOGRdIQ44>:7pc;asl`d03_D3Hhp`sKYZmIAZ^>y0^gdJ5t5;8YvSiOv6FtMF@pBhQ^]Q;1w_dGDnu01pFX56rrktANUy]prB?MfIwmr`hTJu4Js\4JaNhdaT]Bre:f`n]Pz?sfHb>l0W0^6R0i38>S2o72f"
559,1
928,0
593,
594,
...
pCaptionType,"REQUIRED: set Caption as String or Alias"
577,0
578,0
579,0
580,0
...
If( Trim( pCaptionType ) @<> 'S' & Trim( pCaptionType ) @<> 'A' );
  nErrors = nErrors + 1;
EndIf;
...
IF( CubeExists( '}CubeAttributes' ) = 0 );
  CubeAttrInsert( '', 'Caption', pCaptionType);
ELSEIF( DimIx( '}CubeAttributes', 'Caption' ) = 0 );
  CubeAttrInsert( '', 'Caption', pCaptionType);
ENDIF;
IF( CubeExists( '}DimensionAttributes' ) = 0 );
  DimensionAttrInsert( '', 'Caption', pCaptionType);
    ...
    If( Scan( '*', sDim ) = 0);
        ...
        Else;
            ...
            IF( CubeExists( sAttrDim ) = 0 );
              AttrInsert( sDim, '', 'Caption', pCaptionType );
