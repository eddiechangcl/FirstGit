601,100
602,"}APQ.Dim.Element.SafeDelete.0.Main"
562,"VIEW"
586,"}APQ Dimension Use"
585,"}APQ Dimension Use"
564,"apliqo_reader"
565,"mDJibXS5NMPyAnAv]SeON5PRXWN5njHBsbUtv;F`JnkA9|pPNT?Cj6qLmMA8\vOcGv6i|mx1biCDdCHdm8_<yuvY99:3Ek2mKEkC6r96AJ5>H1cIUr:MW7Bf\JI8Lv1EncGS??1@hwQT?X1_6d<ud9ER4k1:I^iQ2I]BemyYoqu]eoptb5PM3ppfS6dQ?r1dAupn@a2O"
559,0
928,0
593,
594,
...
########################################################################################
### PURPOSE:
### This process performs a "safe delete" of an element.
### That is the element will only be deleted if no data is stored against the element. Otherwise a warning is produced.
########################################################################################
...
### Params
sProcLogParams = 'pDim:' | pDim |' & '| 'pEle:' | pEle;
### Params
...
cCubSrc         = '}APQ Dimension Use';
cCubParam       = '}APQ Settings';
cDimTest        = '}APQ Escape Characters';
cSubProc        = '}APQ.Dim.Element.SafeDelete.1.TestCubeData';
...
IF( DimIx( pDim, pEle ) = 0 );
  nErr = nErr + 1;
  sErr = sErr | IF( sErr @<> '', ' & ', '' ) | 'Invalid element:' | pEle;
Else;
...
sFilter = Expand('}APQ Dimensions%sDelimEleStart%%pDim%%sDelimDim%}APQ Dimension Use Measure%sDelimEleStart%Position');
...
nRet = 0;
IF( AttrS( '}APQ Cubes', vCub, 'SkipElementDataCheck' ) @<> 'TRUE' );
   nRet = ExecuteProcess( cSubProc, 'pCube', vCub, 'pDim', pDim, 'pEle', pEle );
   IF( nRet = ProcessExitByBreak );
      cCubeList = cCubeList | IF( Long( cCubeList ) > 1, ' & ', '' ) | vCub;
   EndIF;
EndIF;
...
IF( pDebug @>= '1' & cCubeList @<> '' );
   AsciiOutput( sDebugFile, cCubeList );
EndIF;
...
IF( cCubeList @<> '' );
   ItemReject( 'Data exists for element ' | pDim |':'| pEle | ' in cubes ' | cCubeList );
ElseIF( nErr = 0 );
   IF( pDebug @>= '1' );
      AsciiOutput( sDebugFile, 'No cube data found. Deleting element ' | pDim |':'| pEle ); 
   EndIF;
   DimensionElementDeleteDirect( pDim, pEle );
   DimensionUpdateDirect( pDim );
EndIF;
